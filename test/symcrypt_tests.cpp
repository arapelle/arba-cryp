...
std::size_t compute_diversity(cryp::symcrypt::crypto_key& key, uint32_t seed)
{
    std::vector<uint8_t> data;
    data.reserve(256 + 9);
    data.resize(256, 0);
    cryp::symcrypt symcrypt(key, rand::urng_u8<0, 255>(seed));
    symcrypt.encrypt(data);
    ...
    std::cout << number_of_positive_counters << std::endl;
    return number_of_positive_counters;
}

double compute_average_diversity(std::vector<std::size_t>& fails)
{
    rand::urng_u8<0, 255> rng(42);
    cryp::symcrypt::crypto_key key{ 0 };
    std::vector<std::size_t> diversity_indexes;
    unsigned max_i = 10, max_j = 10;
    diversity_indexes.reserve(max_i * max_j);

    for (unsigned i = 0; i < max_i; ++i)
    {
        std::ranges::generate(key, rng);
        for (unsigned j = 0; j < max_j; ++j)
        {
            uint32_t seed = rng();
            std::size_t diversity_index = compute_diversity(key, seed);
            if (diversity_index < 256 * 0.60)
                fails.push_back(diversity_index);
            diversity_indexes.push_back(diversity_index);
        }
    }

    double avg = std::reduce(diversity_indexes.cbegin(), diversity_indexes.cend()) / double(diversity_indexes.size());
...
//    EXPECT_GT(compute_diversity(key, 21), 256 * 0.60);
//    EXPECT_GT(compute_diversity(key, 42), 256 * 0.60);
//    EXPECT_GT(compute_diversity(key, 84), 256 * 0.60);
//    cryp::symcrypt::crypto_key key2{ 0x26, 0x89, 0xd9, 0xbd, 0x96, 0x26, 0x40, 0x23,
//                                    0x88, 0x42, 0xd2, 0x44, 0xd4, 0x8f, 0xe3, 0x00 };
//    EXPECT_GT(compute_diversity(key2, 21), 256 * 0.60);
//    EXPECT_GT(compute_diversity(key2, 42), 256 * 0.60);
//    EXPECT_GT(compute_diversity(key2, 84), 256 * 0.60);
//    cryp::symcrypt::crypto_key key3{ 0x00, 0x00, 0x00, 0x00, 0xab, 0x58, 0x78, 0xff,
//                                     0x00, 0x00, 0x00, 0x00, 0xab, 0x58, 0x78, 0xff };
//    EXPECT_GT(compute_diversity(key3, 7852), 256 * 0.60);
//    EXPECT_GT(compute_diversity(key3, 4580), 256 * 0.60);
//    EXPECT_GT(compute_diversity(key3, 1851), 256 * 0.60);
    std::vector<std::size_t> fails, expected_fails;
    EXPECT_GT(compute_average_diversity(fails), 256 * 0.60);
//    EXPECT_EQ(fails, expected_fails);
}
